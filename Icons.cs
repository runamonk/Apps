...
namespace Icons
{
    internal class IconFuncs
    {
        ...
        public static Image GetIcon(string fileName, string iconIndex)
        {
            ...
            if (File.Exists(fileName) || fileName.StartsWith(ShellAppPrefix))
            {
                ...
                if (string.IsNullOrEmpty(iconIndex) || iconIndex == "0")
                    return new Bitmap(Icon.ExtractAssociatedIcon(fileName).ToBitmap());

                var i = GetIconEx(fileName, Convert.ToInt32(iconIndex));
                return i?.ToBitmap();
            }
        ...
        public static Icon GetIconEx(string fileName, int index)
        {
            try
            {
                ExtractIconEx(fileName, index, out var large, out var small, 1);
